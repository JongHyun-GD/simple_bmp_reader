...
/*   Updated: 2021/06/20 20:00:31 by jongpark         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		read_pixels(int fd, t_bmp *bmp)
{
	...
	if (read(fd, buf1, bmp->data_offset - 54) == -1 | read(fd, buf2, bmp->size - bmp->data_offset) == -1)
		return (-1);
	...
	while (i < bmp->height * bmp->width)
	{
		...
		//printf("[i = %x] %x, %x, %x\n", i*3, bmp->pixels[i].r, bmp->pixels[i].g, bmp->pixels[i].b);
		i++;
	}
	return (0);
}
...
t_bmp	*read_bmp(char *filename)
{
	...
	if (read_file_header(fd, bmp) || read_info_header(fd, bmp) || read_pixels(fd, bmp))
	{
		perror("failed to read bmp.\n");
		return (0);
	}
	printf("bmp file \"%s\" read ======<\n", filename);
	printf("size:\t%u\n", bmp->size);
	printf("offset:\t%u\n", bmp->data_offset);
	printf("width:\t%u\n", bmp->width);
	printf("height:\t%u\n", bmp->height);
